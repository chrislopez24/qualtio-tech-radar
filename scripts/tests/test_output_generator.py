...
def test_output_generator_creates_full_internal_file(tmp_path, sample_technologies):
    """Test that generate_outputs creates a full internal file with all data"""
    output_dir = tmp_path / "data"
    output_dir.mkdir()
    ...
    assert full_file.exists(), "Full internal file should be created"

    with open(full_file) as f:
        full_payload = json.load(f)

    assert "technologies" in full_payload
    assert len(full_payload["technologies"]) == len(sample_technologies)
