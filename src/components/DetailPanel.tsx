...
function getDesktopAnchorStyle(anchor: { x: number; y: number } | null): React.CSSProperties {
  ...
  const attachLeft = anchor.x <= 470;
  const horizontalOffset = `${(anchor.x / 800) * 100}%`;
  const verticalOffset = `${(anchor.y / 800) * 100}%`;
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.aside
            initial={{ opacity: 0, scale: 0.96, x: anchor && anchor.x > 470 ? -14 : 14, y: -8 }}
            animate={{ opacity: 1, scale: 1, x: 0, y: 0 }}
            exit={{ opacity: 0, scale: 0.96, x: anchor && anchor.x > 470 ? -14 : 14, y: -8 }}
